...
const useTheme = (theme, pathOrValue) =>
  ...
  for (let [name, value] of Object.entries(attributes)) {
    ...
    for (let cssProp of name) {
      ...
      if (!Array.isArray(value)) {
        cssText += `${cssProp}:${useTheme(theme, value)};`;
        continue;
      }

      // responsive definitions
      cssText += `${cssProp}:${useTheme(theme, value[0])};`;
      if (value.length > 1) {
        for (let i = 0; i < value.length; i++) {
          ...
          mediaQueries[i] += `${cssProp}:${useTheme(theme, value[i])};`;
...
export { css, createCssText, useTheme, styled };